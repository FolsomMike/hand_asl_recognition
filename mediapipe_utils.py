...
def decode_bboxes(score_thresh, rawScores, bboxes, anchors):
    ...
    # w = w * anchor.w / wi (in the previous line, we add anchor.x_center and anchor.y_center to w and h,
    #   we need to subtract them now)
    # h = h * anchor.h / hi
...
def rotated_rect_to_points(cx, cy, w, h, rotation, wi, hi):
    ...
    # avoid unused warning, but keep these for now
    wi += 0
    hi += 0
...
def calculateAngleBetween3Points(a, b, c):
